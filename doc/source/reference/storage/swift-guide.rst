...
Swift Storage network (``swift_storage_interface``)
  This network is used to access the account, container and object servers.
  Defaults to ``storage_interface``.
Swift replication network (``swift_replication_network``)
  This network is used for storage replication traffic. This is optional as the
  default configuration uses the swift storage network for replication traffic.
